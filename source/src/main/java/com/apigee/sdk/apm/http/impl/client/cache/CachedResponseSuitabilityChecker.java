 ...
 * response for a given {@link HttpRequest}.
 * 
 * @since 4.1
 */
@Immutable
class CachedResponseSuitabilityChecker {
	...
	CachedResponseSuitabilityChecker(
			final CacheValidityPolicy validityStrategy, CacheConfig config) {
		super();
	...
	private boolean isFreshEnough(HttpCacheEntry entry, HttpRequest request,
			Date now) {
			...
			for (HeaderElement elt : ccHdr.getElements()) {
				...
				if (HeaderConstants.CACHE_CONTROL_MAX_AGE.equals(elt.getName())) {
					try {
						...
						// err conservatively
						log.debug("Response from cache was malformed: "
								+ ex.getMessage());
						return false;
					}
				}

				if (HeaderConstants.CACHE_CONTROL_MAX_STALE.equals(elt
						.getName())) {
					try {
				...
				if (HeaderConstants.CACHE_CONTROL_MIN_FRESH.equals(elt
						.getName())) {
					try {
						long minfresh = Long.parseLong(elt.getValue());
						if (minfresh < 0L)
							return false;
						long age = validityStrategy.getCurrentAgeSecs(entry,
								now);
						long freshness = validityStrategy
	...
	public boolean isConditional(HttpRequest request) {
		return hasSupportedEtagVadlidator(request)
				|| hasSupportedLastModifiedValidator(request);
	}
	 ...
	 * @return
	 */
	public boolean allConditionalsMatch(HttpRequest request,
			HttpCacheEntry entry, Date now) {
		...
		boolean etagValidatorMatches = (hasEtagValidator) ? etagValidtorMatches(
				request, entry) : false;
		boolean lastModifiedValidatorMatches = (hasLastModifiedValidator) ? lastModifiedValidatorMatches(
				request, entry, now) : false;
	 ...
	 * @return
	 */
	private boolean lastModifiedValidatorMatches(HttpRequest request,
			HttpCacheEntry entry, Date now) {
		Header lastModifiedHeader = entry
				.getFirstHeader(HeaderConstants.LAST_MODIFIED);
		Date lastModified = null;
		try {
			if (lastModifiedHeader != null) {
				lastModified = DateUtils.parseDate(lastModifiedHeader
						.getValue());
			}
