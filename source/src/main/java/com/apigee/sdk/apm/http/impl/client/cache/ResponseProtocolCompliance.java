...
class ResponseProtocolCompliance {
	...
	public void ensureProtocolCompliance(HttpRequest request,
		...
		authenticationRequiredDidNotHaveAProxyAuthenticationHeader(request,
				response);
	...
	private void warningsWithNonMatchingWarnDatesAreRemoved(
		...
		try {
			responseDate = DateUtils.parseDate(response.getFirstHeader("Date")
					.getValue());
		...
		for (Header h : warningHeaders) {
			for (WarningValue wv : WarningValue.getWarningValues(h)) {
				...
				if (warnDate == null || warnDate.equals(responseDate)) {
					newWarningHeaders.add(new BasicHeader("Warning", wv
							.toString()));
				} else {
					modified = true;
				}
	...
	private void identityIsNotUsedInContentEncoding(HttpResponse response) {
		...
		for (Header h : hdrs) {
			...
			if (!"".equals(newHeaderValue)) {
				newHeaders.add(new BasicHeader("Content-Encoding",
						newHeaderValue));
			}
		}
		if (!modified)
	...
	private void unauthorizedResponseDidNotHaveAWWWAuthenticateHeader(
			HttpRequest request, HttpResponse response)
			throws ClientProtocolException {
	...
	private void ensurePartialContentIsNotSentToAClientThatDidNotRequestIt(
			HttpRequest request, HttpResponse response)
			throws ClientProtocolException {
	...
	private boolean backendResponseMustNotHaveBody(HttpRequest request,
			HttpResponse backendResponse) {
		return HeaderConstants.HEAD_METHOD.equals(request.getRequestLine()
				.getMethod())
